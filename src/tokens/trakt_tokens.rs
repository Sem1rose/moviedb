...
impl TraktTokens {
    ...
    fn check_files(&mut self, home_dir: &PathBuf) -> anyhow::Result<bool> {
        if !home_dir.join(".key").is_file() {
            ...
            _ = fs::remove_file(&home_dir.join(".credentials_trakt"));

            fs::write(&home_dir.join(".key"), key)?;
        }

        Ok(home_dir.join(".credentials_trakt").is_file())
    }
    ...
    fn read_creds(home_dir: &PathBuf) -> anyhow::Result<String> {
        ...
        let mut encrypted_file = File::open(&home_dir.join(".credentials_trakt"))?;
    ...
    pub fn save_creds(&self, home_dir: &PathBuf) -> anyhow::Result<()> {
        ...
        let mut encrypted_file = File::create(&home_dir.join(".credentials_trakt"))?;
